...
iHP=1
decompose_inflow_data=decompose(ts_inflow_data[,iHP],"additive") #has to be a time series object, easier for it to understand and separate the series into 3
...
cat(mean_inflow,sd_inflow) #Note random series has some missing values, that is why we got NAs
...
inflow_random <- data.frame(date=my_date,month=as.factor(month(my_date)),inflow=as.numeric(inflow_random)) #ensure data is numeric

#How many NAs we have, you can get it from summary or using is.na()
sum(is.na(inflow_random$inflow)) # sum the amount of times that there are NAs (e.g. sum NA=true), there are 12 NAs in this dataset
...
#Check data again
sum(is.na(inflow_random$inflow)) #removed them bc they were at the end of the dataset
head(inflow_random,10)
tail(inflow_random,10)
...
ggplot(inflow_random, aes(x=month, y=inflow)) +
            geom_boxplot() #other options: geom_line, geom_point

ggplot(inflow_random, aes(x=date, y=inflow)) +
            geom_line() +
            geom_abline(slope=0,intercept=3*sd_inflow,color="red") + #plus 3 standard deviation
            geom_abline(slope=0,intercept=-3*sd_inflow,color="red") #minus 3 standard deviation

#everything outside of the red lines could be tested for being outliers bc it outside of 3 standard deviations

ggplot(inflow_random, aes(y=inflow)) +
            geom_boxplot() #all the data, excluding month split
```
...
#Since we removed seasonality replacing with overall mean instead of local mean is acceptable,  do not fill with mean if seasonality is present
...
while(pvalue < 0.05){ #the algorithm only enter the loop if the p-value 
  ...
  out_test <- grubbs.test(aux_inflow,type=10) #10 is looking for the largest outlier, 11 tests for two outliers (max and min), 20 is test for two outliers in one tail
