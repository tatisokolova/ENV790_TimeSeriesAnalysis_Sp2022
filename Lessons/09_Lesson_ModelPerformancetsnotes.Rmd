...
checkresiduals(MEAN_seas) #residuals is difference between observed and fitted (observed minus mean of series)
```
...
checkresiduals(SNAIVE_seas) #no trend, may be biased (overstimating the value); no clear seasonal trend but still not good because it has some of the long memory property (ACF shows AR)
```
...
Code was changed to access foercast accuracy

```{r}
observed <- electricity_price_processed[(nobs-n_for+1):nobs, "All.sectors"]
#Model 1: Arithmetic mean
#MEAN_scores0 <- accuracy(MEAN_seas,observed)  #includes the training and test (remove mean)
#plot(MEAN_seas)  #plot forecasts

MEAN_scores <- accuracy(MEAN_seas$mean, observed)  #store the performance metrics, only includes test
plot(MEAN_seas)  #plot forecasts
...
```{r}
#Create a data frame where first column is observed and the other columns correspond to the forecast for each of the three models.

combined <- data.frame("Date" = electricity_price_processed$Month[(nobs-n_for+1):nobs],observed,MEAN_seas$mean,SNAIVE_seas$mean,SARIMA_for$mean)
head(combined)

#use ggplot to show observed and forecast on a single plot and ADD LEGEND
...
ggplot(combined, aes(x=Date)) +
    ...
    geom_line(aes(y=SARIMA_for.mean), color = "green")
...
#autoplot reads time series so you don't have to go through df/ggplot mess

autoplot(ts_electricity_price_full) +
    autolayer(SARIMA_for, series="SARIMA", PI=FALSE) +  #autolayer instead of geom_line
    autolayer(MEAN_seas, series = "Arithmetic Mean", PI=FALSE) + #looks for mean bc it knows it's time series object
