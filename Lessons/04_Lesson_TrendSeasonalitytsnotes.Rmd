...
#dates start at 1970 in R, dates before 1970 are nefgative
...
ggplot(inflow_data, aes(x=my_date, y=inflow_data[,(iHP+1)])) +
            geom_line(color="blue") +
            ylab(paste0("Inflow ",colnames(inflow_data)[(iHP+1)],sep="")) +
            #geom_abline(intercept = beta0, slope = beta1, color="red") #don't use this for now
...
#detrend to remove any trends and just find the variability of the data
...
seas_means_model=lm(inflow_data[,(iHP+1)]~dummies) #december is missing bc represented by the intercept term, can run with 12 but otherwise have to run regression without intercept

summary(seas_means_model)
#wet period is summer, dry period in winter, hence why Jan-Apr is pos and May-Nov is dry
