...
library(ggplot2)
library(forecast)
library(Kendall) #pearson is regular correlation (high school), spearman is similar to kendall but works with non-linear relationships, kendall is monotic as well as spearman but less affected by outliers
library(tseries)
...
print(summary(SMKtest)) #score = big S, since it's a large positive number, it's a positive trend

#Use yearly date to run Mann Kendall
print("Results of Mann Kendall on average yearly series")
print(summary(MannKendall(inflow_data_yearly))) #score is lower because there are less observations, p value is sharign how significant teh tau value is, it definitely statistically different from 0 so we will reject the null hypothesis
#??corr to look up topics
#3160, how many times you computed S
#618 is how many times you got the result of 1 (over -1 or 0)
```
...
sp_rho=cor.test(inflow_data_yearly,my_year,method="spearman") #use the cor.test, not just rho value to get the p value to support the rho value output; note: cannot handle seasonal data; always need to group data to eliminate seasonality first
print(sp_rho)
```
...
print(adf.test(ts_inflow_data[,iHP],alternative = "stationary")) #stationary over a unit root but could be non stationary 
