...
> Think about what kind of datasets are affected by seasonality. Electricity prices are affected by weekly (e.g. week vs weekend). Weather data is not affected by weekly trends
...
> Add seasonality as regressors, do not fit for SARIMA
> state equations represent parameters of your model. Each parameter has an equation
> Neural network aut-regressive package trained with previous obsvervations of our series: simple model but handles non-linear relationships. ARIMA only looks at linear relationships. 
...
ts_act_power_daily <- msts(act_power_daily$daily_mean_active_power, 
                           seasonal.periods =c(7,365.25), #two seasonal periods instead of 3 bc hourly data is removed
                           start=c(2006,12,16))
...
>stl is like decompose, can only handle on seasonal component. mstl can handle msts objects
...
ts_act_power_daily %>% mstl() %>% #pipes strucutre allows you to only call the times series once and then can call the various functions for it.
  autoplot()
...
autoplot(ETS_fit) + ylab("Active Power") #ANN = additive, no trend, non-seasonal
